package body Logging is
   ...
   protected body Logging_Termination_Handler is
      ...
      procedure Log_Termination_Cause
        (C : Cause_Of_Termination; T : Task_Id; X : Exception_Occurrence) is
      begin
         case C is
            when Normal =>
               null;
            when Abnormal =>
               IO.Put_Line
                 (IO.Current_Error,
                  W ("Something caused termination in task " & Image (T)));
            when Unhandled_Exception =>
               IO.Put_Line
                 (IO.Current_Error,
                  W ("Uncaught exception terminated task " & Image (T) & ":"));
               IO.Put_Line
                 (IO.Current_Error,
                  W (Exception_Information (X)));
         end case;
      end Log_Termination_Cause;
